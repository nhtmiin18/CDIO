            ...
            <div className="max-w-7xl mx-auto p-8 space-y-8">
                ...
                {loading && (
                    <div className="text-center py-20">
                        ...
                        <p className="text-xl font-semibold">Analyzing your uploaded CV...</p>
                    </div>
                )}

                {!loading && !data && (
                    <div className="text-center py-20 bg-white rounded-lg shadow">
                        <h3 className="text-xl text-red-500">No CV Found</h3>
                        <p>Please go to the Upload screen to submit your CV first.</p>
                    </div>
                )}
...
function SkillGroup({ title, items }) {
    return (
        <div className="mb-4">
            <div className="text-sm font-medium mb-2 text-gray-600">{title}</div>
            <div className="flex flex-wrap gap-2">
                {items.map((i) => (
                    <span key={i} className="border border-gray-200 bg-gray-50 px-3 py-1 text-sm rounded-md text-slate-700">
                        {i}
                    </span>
                ))}
            </div>
...
function MatchedPosition({ title, score }) {
    return (
        <div className="border p-4 mb-3 flex justify-between items-center rounded-lg bg-gray-50">
            <div className="font-medium text-slate-700">{title}</div>
            <div className={`font-bold ${score >= 90 ? 'text-green-600' : 'text-blue-600'}`}>
                {score}%
            </div>
        </div>
    );
